...
extern kern_return_t mach_exc_server(
...
void *exception_listener(void *arg) {
    ...
    for (;;) {
        ...
        if (kr != KERN_SUCCESS) {
            fprintf(stderr,
                    "[-] exception_listener: mach_msg receive failed: %s\n",
                    mach_error_string(kr));
            printf("phantom> ");
            fflush(stdout);
            continue;
        ...
        boolean_t handled = mach_exc_server(
            (mach_msg_header_t *)&in_msg,
            (mach_msg_header_t *)&out_msg
        );
        if (!handled) {
            fprintf(stderr,
                    "[-] exception_listener: dispatch failed\n");
            printf("phantom> ");
            fflush(stdout);
            continue;
        }
        ...
        kr = mach_msg(
            (mach_msg_header_t *)&out_msg,
            MACH_SEND_MSG,
            out_msg.Reply_mach_exception_raise.Head.msgh_size,
            0,
            MACH_PORT_NULL,
            MACH_MSG_TIMEOUT_NONE,
            MACH_PORT_NULL
        ...
        if (kr != KERN_SUCCESS) {
            fprintf(stderr,
                    "[-] exception_listener: mach_msg send failed: %s\n",
                    mach_error_string(kr));
        }
    }
